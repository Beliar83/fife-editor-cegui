...
class Behaviours(Dialog):
    ...
    """Class that displays a behaviours dialog"""
    ...
    def setup_dialog(self, root):
        ...
        ed_available = layout.createChild("TaharezLook/Button",
                                          "EditAvaible")
        ed_available.setText(_("Edit Available behaviours"))
        ...
        ed_available.subscribeEvent(PyCEGUI.ButtonBase.EventMouseClick,
                                    self.cb_edit_available_behaviours_clicked)

        self.project_behaviours = set(self.app.project.get("fife-rpg",
                                                           "Behaviours", []))
        self.update_lists()
    ...
    def update_lists(self):
        ...
        self.available_items = []
        self.used_bases = set()
        current_behaviours = set()

        for behaviour in self.project_behaviours:
            behaviour_class = self.app.get_behaviour_data(behaviour)[0]
            for dependency in behaviour_class.dependencies:
                current_behaviours.add(dependency.__name__)
            for base in behaviour_class.__bases__:
                if base is BaseBehaviour:
                    continue
                self.used_bases.add(base.__name__)

        for behaviour in self.project_behaviours.copy():
            if behaviour in self.used_bases:
                self.project_behaviours.remove(behaviour)
                continue
        ...
        all_behaviours = set(self.app.behaviours.keys())
        for behaviour in all_behaviours - self.project_behaviours:
            ...
            elif behaviour in current_behaviours:
    ...
    def cb_move_left_clicked(self, args):
        ...
        for item in items:
            ...
            item = self.available_list.createChild("TaharezLook/ListboxItem")
            item.setText(text)
            self.project_behaviours.remove(text)
            self.available_items.append(item)
        self.current_list.performChildWindowLayout()
        self.available_list.performChildWindowLayout()
        self.update_lists()
    ...
    def cb_move_right_clicked(self, args):
        ...
        for item in items:
            ...
            item = self.current_list.createChild("TaharezLook/ListboxItem")
            item.setText(text)
            self.project_behaviours.add(text)
            self.current_items.append(item)
        self.current_list.performChildWindowLayout()
        self.available_list.performChildWindowLayout()
        self.update_lists()
    ...
    def cb_edit_available_behaviours_clicked(self, args):
        """Callback for a click on the 'Edit available behaviours' button"""
        self.app.edit_available_behaviours()
        self.app.current_dialog = self
        self.project_behaviours = (self.project_behaviours &
                                   set(self.app.behaviours.keys()))
        self.update_lists()


class AvailableBehaviours(Dialog):

    """Dialog for editing the available behaviours"""
    ...
    def setup_dialog(self, root):
        ...
        self.window.setMinSize(PyCEGUI.USize(PyCEGUI.UDim(0.4, 3),
                                             PyCEGUI.UDim(0.5, 4)))
        self.window.setText(_("Available Behaviours"))
        ...
        self.items = []
        self.values = dict()
        for name, path in self.app.behaviours.items():
    ...
    def cb_add_clicked(self, args):
        ...
        for member in dir(module):
            ...
            try:
                if (issubclass(behaviour, BaseBehaviour) and
                        behaviour is not BaseBehaviour):
                    behaviour_name = behaviour.__name__
                    if ("." in behaviour.__module__ and
                            behaviour.__module__ != module_path):
                        continue
                    if behaviour_name not in self.values:
                        self.values[behaviour_name] = module_path
                        item = self.edit_set.createChild("TaharezLook/"
                                                         "ListboxItem")
                        item.setText(behaviour_name)
                        self.items.append(item)
            except TypeError:
                pass
    ...
    def get_values(self):
        return {"behaviours": self.values.copy()}
