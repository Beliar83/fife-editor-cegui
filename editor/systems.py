...
import PyCEGUI
from fife_rpg.systems.base import Base as BaseSystem
...
class Systems(Dialog):
    ...
    """Class that displays a systems dialog"""
    ...
    def setup_dialog(self, root):
        ...
        ed_available = layout.createChild("TaharezLook/Button",
                                          "EditAvaible")
        ed_available.setText(_("Edit Available systems"))
        ...
        ed_available.subscribeEvent(PyCEGUI.ButtonBase.EventMouseClick,
                                    self.cb_edit_available_systems_clicked)

        self.project_systems = set(self.app.project.get("fife-rpg",
                                                        "Systems", []))
        self.update_lists()
    ...
    def update_lists(self):
        ...
        for system in self.project_systems:
            system_class = self.app.get_system_data(system)[0]
            for dependency in system_class.dependencies:
                current_systems.add(dependency.__name__)
            for base in system_class.__bases__:
                if base is BaseSystem:
                    continue
                self.used_bases.add(base.__name__)

        for system in self.project_systems.copy():
            if system in self.used_bases:
                self.project_systems.remove(system)
                continue
        ...
        for system in all_systems - self.project_systems:
            ...
            elif system in current_systems:
    ...
    def cb_move_left_clicked(self, args):
        ...
        for item in items:
            ...
            item = self.available_list.createChild("TaharezLook/ListboxItem")
            item.setText(text)
            self.project_systems.remove(text)
            self.available_items.append(item)
        self.current_list.performChildWindowLayout()
        self.available_list.performChildWindowLayout()
        self.update_lists()
    ...
    def cb_edit_available_systems_clicked(self, args):
        """Callback for a click on the 'Edit available systems' button"""
        self.app.edit_available_systems()
        self.app.current_dialog = self
        self.project_systems = (self.project_systems &
                                set(self.app.systems.keys()))
        self.update_lists()


class AvailableSystems(Dialog):

    """Dialog for editing the available systems"""
    ...
    def setup_dialog(self, root):
        ...
        self.window.setMinSize(PyCEGUI.USize(PyCEGUI.UDim(0.4, 3),
                                             PyCEGUI.UDim(0.5, 4)))
        self.window.setText(_("Available Systems"))
    ...
    def cb_add_clicked(self, args):
        ...
        for member in dir(module):
            ...
            try:
                if (issubclass(system, BaseSystem) and
                        system is not BaseSystem):
                    system_name = system.__name__
                    if ("." in system.__module__ and
                            system.__module__ != module_path):
                        continue
                    if system_name not in self.values:
                        self.values[system_name] = module_path
                        item = self.edit_set.createChild("TaharezLook/"
                                                         "ListboxItem")
                        item.setText(system_name)
                        self.items.append(item)
            except TypeError:
                pass
    ...
    def get_values(self):
        return {"systems": self.values.copy()}
