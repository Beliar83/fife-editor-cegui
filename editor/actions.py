...
"""Contains classes and functions for the actions dialog"""
...
class Actions(Dialog):
    ...
    """Class that displays a actions dialog"""
    ...
    def setup_dialog(self, root):
        ...
        ed_available = layout.createChild("TaharezLook/Button",
                                          "EditAvaible")
        ed_available.setText(_("Edit Available actions"))
        ...
        ed_available.subscribeEvent(PyCEGUI.ButtonBase.EventMouseClick,
                                    self.cb_edit_available_actions_clicked)

        self.project_actions = set(self.app.project.get("fife-rpg",
                                                        "Actions", []))
        self.update_lists()
    ...
    def update_lists(self):
        ...
        for action in self.project_actions:
            action_class = self.app.get_action_data(action)[0]
            for dependency in action_class.dependencies:
                current_actions.add(dependency.__name__)
            for base in action_class.__bases__:
                if base is BaseAction:
                    continue
                self.used_bases.add(base.__name__)

        for action in self.project_actions.copy():
            if action in self.used_bases:
                self.project_actions.remove(action)
                continue
        ...
        all_actions = set(six.iterkeys(self.app.actions))
        for action in all_actions - self.project_actions:
            item = self.available_list.createChild("TaharezLook/ListboxItem")
            text = action
            if action in self.used_bases:
                text = "[colour='FF0000FF']" + text
            elif action in current_actions:
    ...
    def cb_move_left_clicked(self, args):
        ...
        for item in items:
            ...
            item = self.available_list.createChild("TaharezLook/ListboxItem")
            item.setText(text)
            self.project_actions.remove(text)
            self.available_items.append(item)
        self.current_list.performChildWindowLayout()
        self.available_list.performChildWindowLayout()
        self.update_lists()
    ...
    def cb_edit_available_actions_clicked(self, args):
        """Callback for a click on the 'Edit available actions' button"""
        self.app.edit_available_actions()
        self.app.current_dialog = self
        self.project_actions = (self.project_actions &
                                set(self.app.actions.keys()))
        self.update_lists()


class AvailableActions(Dialog):

    """Dialog for editing the available actions"""
    ...
    def setup_dialog(self, root):
        ...
        self.window.setMinSize(PyCEGUI.USize(PyCEGUI.UDim(0.4, 3),
                                             PyCEGUI.UDim(0.5, 4)))
        self.window.setText(_("Available Actions"))
        ...
        self.items = []
        self.values = dict()
        for name, path in six.iteritems(self.app.actions):
        ...
        for member in dir(module):
            ...
            try:
                if (issubclass(action, BaseAction) and
                    ...
                    if ("." in action.__module__ and
                            action.__module__ != module_path):
                        continue
                    if action_name not in self.values:
                        self.values[action_name] = module_path
    ...
    def get_values(self):
        return {"actions": self.values.copy()}
